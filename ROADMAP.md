...
| PF4 | A → B → A → undo acceptance pass across Theme, Sounds, Enemy and Profile on the real game | **Passed** (2026-09-22) on `0.1.4-beta.2` from `856fb40`, by the user on the tester's PC: A → B → A on all four, the Beta mark, the tuner, 应用并启动游戏 and the Profile sheets' search (「都对」). The going-back path is re-applying the earlier choice; the page button that drops an unapplied choice is renamed 退出 / Exit (user: it always meant leaving, not undo) |
